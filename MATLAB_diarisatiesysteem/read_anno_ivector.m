function [labels_spk,VAD,begin_seq,end_seq] = read_anno_ivector(filename,precision,windowlength)
...
temp = readout{1};
min_Boundaries = baseprecision*round(temp(2:end)./baseprecision);
%min_Boundaries = [0; precision*round((temp(3:end)-0.200)./precision)];
begin_Sequence = temp(1);
temp = readout{2};
end_Sequence = temp(1) ;
max_Boundaries = baseprecision*round(temp(2:end)./baseprecision);
%max_Boundaries = precision*round((temp(2:end)-0.200)./precision);
temp = readout{3};
labels_ID = temp(2:end);
begin_seq = baseprecision*round(begin_Sequence/baseprecision);
end_seq = baseprecision*round(end_Sequence/baseprecision);
%annotation_Length = round(((end_seq - begin_seq))/baseprecision);
%labels_spk = zeros(1,annotation_Length);
seg_lengths = round((max_Boundaries - min_Boundaries)./baseprecision);
...
i = 1;
windowL = round(windowlength/baseprecision);
startL = 1;
finL = windowL;
Seq = labeled_GroundTruth(startL:finL);
labels_spk(i) = mode(Seq);
hopL = round(precision/baseprecision);
