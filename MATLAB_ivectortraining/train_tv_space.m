...
for iter = 1 : niter,
    ...
    tim = tic;
    [LU, RU] = expectation_tv(T, dataList, S, tv_dim, nmix, ndim, nworkers);
...
function [LU, RU] = expectation_tv(T, dataList, S, tv_dim, nmix, ndim, nworkers)
...
for batch = 1 : nbatch
    ...
    for i = 1:length(index)
        BWstats_fname = [pwd, '/BWstats/', dataList{index(i)}];
        temp = load(BWstats_fname);
        N1 = [N1; temp.N.'];
        F1 = [F1; temp.F.'];
    end
    %N1 = N(index, :);
