...
%   - nworkers    : numbe r of parallel workers
...
while ( mix <= nmix )
    ...
    for iter = 1 : niter(log2(mix) + 1)
        ...
        for ix = 1:nfiles
%         for ix = 1 : nfiles,
            datapath = [pwd, '/MELfeatures/',dataList{ix}];
            temp = cell2mat(struct2cell(load(datapath)));
            temp = temp(:,1:ds_factor:end);
            [n, f, s, l] = expectation(temp, gmm);
...
data = cell(nfiles, 1);
for i = 1:nfiles
    data(i) = struct2cell(load(datalist{i},'-mat'));
...
fileInf = {};
for i =1:nr_data
    datapath = [pwd, '/MELfeatures/', data{i}]; 
    temp = cell2mat(struct2cell(load(datapath)));
    %sumNaN = sumNaN + sum(isnan(temp(:)));
    %if sum(isnan(temp(:))) ~= 0
    %    fileNaN = fileNaN + 1;
    %end
    nframes = nframes + size(temp,2);
    %test = sum(temp,2)/nframes;
    %if any(isinf(test(:)))
    %    fileInf = [fileInf, {data{i}}];
    %end
    gm = gm + sum(temp,2);
end
gm = gm/nframes;
